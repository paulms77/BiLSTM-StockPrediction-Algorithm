...
def get_adjust_close(dfs:pd.DataFrame(), symbol:str, column:str, gap_period: int, cummulative: bool)->pd.DataFrame():
    """
        수정주가 계산
        Args:
            dfs (pd.DataFrame): 수정주가 계산을 수행할 원본 데이터
            symbol (str): 종목코드
            column (str): 수정주가 계산을 수행할 컬럼
            gap_period (int): 공백 기간
            cummulative (bool): 누적 이벤트 반영
        Returns:
            pd.DataFrame: 수정주가 계산을 수행 후 결과 데이터
    """
    adj_column = 'ADJ_' + column
    dfs[adj_column] = 0
    adjust_ratios = []
    adjust_ratio = 1.0
    adjust_ratios.append(adjust_ratio)
    #gap_period = datetime.timedelta(days=gap_period)
    for i in range(1, dfs.shape[0]):
        if dfs.index[i] - dfs.index[i-1] > gap_period:
            adjust_ratio = 1.0
        else:
            standard_close = dfs.loc[i, column]-dfs.loc[i, 'CHANGE']*adjust_ratio
            if cummulative:
                adjust_ratio *= standard_close/dfs.loc[i-1, column]
                #adjust_ratio *= ((1+dfs.loc[i-1, column] - standard_close) / (dfs.loc[i-1, column])) 
            else:
                adjust_ratio = standard_close/dfs.loc[i-1, column]
                #adjust_ratio = ((1+dfs.loc[i-1, column] - standard_close) / (dfs.loc[i-1, column])) 
    ...
    if column == 'VOLUME':
        dfs[adj_column] = dfs[column] / adjust_ratios
    else:
        dfs[adj_column] = dfs[column] * adjust_ratios
